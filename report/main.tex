...
\end{equation}
$A$ is built once from the number of beads, $\alpha$, and $\beta$. It does not depend on where the beads sit.

Figure~\ref{fig:stiffness} is that matrix for a 16-bead loop. The ring shows who bead 0 is tied to: the two neighbors, and the beads two steps away. The heatmap is $A$, with row 0 boxed. Pale blue is zero. The corner squares wrap the loop.

Applying $A$ to the current positions is the internal pull on every bead at once. Figure~\ref{fig:internal} draws that pull on a loop with one wide gap and one kink. These arrows sit on the beads. The red beads bound the gap, and under tension the arrows there point across it, so the gap closes. The blue bead is the kink, and under bending the arrow there points back toward the loop, so the kink flattens.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{stiffness_matrix.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{internal_forces.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{image_energy.png}
  \caption{Line, edge, and termination energy. Darker is lower. The orange line is the plotted row. Dashed: disk. Dotted: square. Dash-dot: stroke. Termination is scaled so the corners and the stroke ends read; larger values saturate.}
...
Figure~\ref{fig:constraints} draws both on the initial circle. Blue is the spring: the line runs to the pin, and the arrow is the pull on that bead. Orange is the volcano: the ring is the crater, and the arrows on the nearby beads point away from it.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{constraints.png}
  \caption{Snake Pit forces on the initial circle. Blue: a spring and its pull toward the pin. Orange: a volcano and the push on beads outside the crater. Arrow lengths are scaled for display.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{iteration.png}
